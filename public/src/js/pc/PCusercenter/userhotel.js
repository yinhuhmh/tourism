...
// 引入自己写好的模块
// 设计好所需的模块
export default class Userhotel extends React.Component{
	constructor(){
		super();
		this.state={
			data:[]
	...
	componentDidMount(){
		fetch('/post_usercollect',{
			...
			body:`userid=${localStorage.userid}&sckey=2`
		}).then((response)=>{
					...
					<div className='collectitem'>
						<div className='collecttable'>
							<div className='collectleft'>
								...
								<div className='text'>
									<Link to={`/pchldetails/${item.title}`}><span className='title'>{item.title}</span></Link>
									<ul>
										<li>
											<span>类型</span>
											<i>：</i>
							...
							<div className='operation'>
								<Link to={`/pchldetails/${item.title}`}><p>预定</p></Link>
